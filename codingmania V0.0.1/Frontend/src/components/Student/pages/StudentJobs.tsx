...
const JobCard = ({ job }: { job: Job }) => {
  return (
    ...
    >
          ...
          {job.tags && job.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2.5">
              {job.tags.map((t) => (
                <span
                  key={t}
                  className="text-[10px] font-semibold px-2 py-0.5 rounded-lg bg-indigo-50 text-indigo-600 border border-indigo-100"
                >
                  {t}
                </span>
              ))}
          ...
          <div className="flex items-center gap-4 mt-3 flex-wrap">
            {job.poster && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] text-gray-500 font-medium">{job.poster.name}</span>
              </div>
            )}
      ...
      <div className="flex items-center justify-end mt-4 pt-3 border-t border-gray-100/60">
        {job.applicationLink ? (
          <a
          ...
          <span className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-gray-400 bg-gray-100 rounded-xl cursor-not-allowed">
            <ExternalLink className="w-3.5 h-3.5" />
  ...
  return (
    <div className="space-y-6">
      ...
      >
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-3 py-2 bg-white/70 border border-gray-200 rounded-xl text-xs text-gray-700 placeholder-gray-400 w-52 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 transition-all"
              />
            </div>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 border rounded-xl text-xs font-medium transition-all ${
                showFilters || typeFilter !== "All"
                  ...
                  : "bg-white/70 border-gray-200 text-gray-500 hover:bg-gray-50"
              }`}
            >
        ...
        {showFilters && (
          <div className="px-6 py-3 border-b border-gray-100/60 flex flex-wrap items-center gap-3">
            <div className="relative">
              <select
                value={typeFilter}
                onChange={(e) => setTypeFilter(e.target.value)}
                className="appearance-none bg-white/70 border border-gray-200 rounded-xl text-xs text-gray-700 pl-3 pr-8 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 cursor-pointer"
              >
        ...
        <div className="px-6 py-2.5 border-b border-gray-100/40 flex items-center gap-4">
