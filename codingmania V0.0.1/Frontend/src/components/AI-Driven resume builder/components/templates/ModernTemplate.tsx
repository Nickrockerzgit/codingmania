...
export const ModernTemplate: React.FC<ModernTemplateProps> = ({ data }) => {
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden max-w-4xl mx-auto">
      {/* Header */}
      <div className="bg-gradient-to-r from-red-600 to-orange-600 text-white p-4 sm:p-6 lg:p-8">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-2">{personalInfo.fullName}</h1>
        <p className="text-red-100 text-sm sm:text-base lg:text-lg mb-4">{personalInfo.professionalSummary}</p>
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-1 space-y-4 lg:space-y-6">
            ...
            {skills.length > 0 && (
              <section>
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4 border-b-2 border-red-600 pb-2">Skills</h2>
                      ...
                      <div key={category}>
                        ...
                        <div className="flex flex-wrap gap-1 sm:gap-2">
                          {categorySkills.map((skill, index) => (
                            <span key={index} className="px-2 py-1 bg-red-100 text-red-800 text-xs sm:text-sm rounded-full">
                              {skill.name}
                            </span>
                          ))}
                        </div>
            ...
            {(socialLinks.github || socialLinks.linkedin || socialLinks.portfolio) && (
              <section>
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4 border-b-2 border-red-600 pb-2">Links</h2>
                <div className="space-y-2">
                  {socialLinks.github && (
                    <div className="flex items-center gap-2">
                      <Github className="w-4 h-4 text-gray-600" />
                      <span className="text-xs sm:text-sm text-red-600 break-all">{socialLinks.github}</span>
                    </div>
                  )}
                  {socialLinks.linkedin && (
                    <div className="flex items-center gap-2">
                      <Linkedin className="w-4 h-4 text-gray-600" />
                      <span className="text-xs sm:text-sm text-red-600 break-all">{socialLinks.linkedin}</span>
                    </div>
                  )}
                  {socialLinks.portfolio && (
                    <div className="flex items-center gap-2">
                      <ExternalLink className="w-4 h-4 text-gray-600" />
                      <span className="text-xs sm:text-sm text-red-600 break-all">{socialLinks.portfolio}</span>
                    </div>
                  )}
                </div>
              </section>
            ...
            {certifications.length > 0 && (
              <section>
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4 border-b-2 border-red-600 pb-2">Certifications</h2>
          ...
          <div className="lg:col-span-2 space-y-4 lg:space-y-6">
            ...
            {experience.length > 0 && (
              <section>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4 border-b-2 border-red-600 pb-2">Experience</h2>
                <div className="space-y-4">
                  {experience.map((exp) => (
                    <div key={exp.id}>
            ...
            {projects.length > 0 && (
              <section>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4 border-b-2 border-red-600 pb-2">Projects</h2>
                <div className="space-y-4">
            ...
            {education.length > 0 && (
              <section>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4 border-b-2 border-red-600 pb-2">Education</h2>
                <div className="space-y-4">
                  {education.map((edu) => (
                    <div key={edu.id}>
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900">{edu.degree}</h3>
                      <p className="text-red-600 font-medium text-sm sm:text-base">{edu.institution}</p>
            ...
            {achievements.length > 0 && (
              <section>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4 border-b-2 border-red-600 pb-2">Achievements</h2>
                <div className="space-y-3">
